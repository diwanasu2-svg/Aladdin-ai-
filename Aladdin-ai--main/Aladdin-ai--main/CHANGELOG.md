...
## Smart Memory Phase 3 Part 3

- Added memory importance scoring
- Added memory ranking based on similarity, importance and recency
- Added dependency-free embedding manager
- Added SQLite-backed vector store
- Added semantic search integration in `MemoryManager`
- Updated configuration and documentation for Part 3

## [2.1.0] - Phase 3 / Smart Memory Part 2

### Added
- `aladdin_core/project_memory.py` — project records with goals, files,
  status, archiving and auto conversation linking.
- `aladdin_core/location_memory.py` — home / office / favorite / custom
  named locations with optional coordinates and nearest-search.
- `aladdin_core/reminder_memory.py` — persistent reminders with due dates,
  completion tracking and recurring schedules.
- `aladdin_core/calendar_memory.py` — meetings, events, birthdays,
  appointments and important dates with future-integration hooks
  (`source` + `external_id`).
- `aladdin_core/conversation_summary.py` — automatic conversation
  summarization triggered after a configurable message count.
- New `MemoryCfg` options:
  `project_memory_enabled`, `location_memory_enabled`,
  `reminder_memory_enabled`, `calendar_memory_enabled`,
  `conversation_summary_enabled`, `summary_trigger_messages`,
  `summary_max_length` (plus per-layer `*_db_path`).
- `SMART_MEMORY_PHASE3_PART2_README.md` documentation.

### Changed
- `aladdin_core/memory_manager.py` integrates the five new layers with
  convenience methods (`add_project`, `set_home`, `add_reminder`,
  `add_calendar_event`, `record_message`, …) while keeping the full
  Smart Memory Part 1 API intact.
- `aladdin_core/__init__.py` exports the new classes.
- `config.yaml` adds the new memory options (all enabled by default).
- `README.md` references the new module set.

### Backward compatibility
- Voice Core (Parts 1 & 2) and AI Brain modules are unchanged.
- All Part 1 APIs continue to work exactly as before.
- New layers are config-gated; disabling any flag results in safe no-op
  behaviour at the manager level.
